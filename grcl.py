...
from generalframeworks.loss.loss import Attention_Threshold_Loss, Grcl_Loss
...
def main():
    global prototype_mu, prototype_sigma
    ...
    criterion = {'ce_loss': nn.CrossEntropyLoss(ignore_index=-1).cuda(local_rank),
                 ...
                 'prcl_loss': Grcl_Loss(strong_threshold=args.strong_threshold,
                                        num_queries=args.num_queries,
                                        num_negatives=args.num_negatives,
                                        num_positives=config['Prcl_Loss']['num_positives'],
                                        num_generalized = args.generalized_num,
                                        generalized_radius=args.generalized_radius,
                                        temp=config['Prcl_Loss']['temp']).cuda(local_rank)
                }

    ### Build prototype ###
    prototype_mu = torch.ones(config['Network']['num_class'], 256).cuda()
    prototype_sigma = torch.ones(config['Network']['num_class'], 256).cuda()
    ...
    if os.path.exists(args.resume):
        ...
        prototype_mu = torch.tensor(checkpoint['prototype_mu']).cuda()
        prototype_sigma = torch.tensor(checkpoint['prototype_sigma']).cuda()
    else:
        start_epoch = 0
                ...
                torch.save(
                    {
                        ...
                        'prototype_mu': prototype_mu.data.cpu().numpy(),
                        'prototype_sigma': prototype_sigma.data.cpu().numpy()
...
def train(train_l_loader, train_u_loader, model, rank, local_rank, world_size, optimizer, optimizer_uncer, criterion, epoch, scheduler, scheduler_uncer, config, args):
    ...
    mious_conf_l = ConfMatrix(num_classes=num_class, fmt=':6.3f', name='l_miou') # Ours, and we redefine the __str__ function to fit progress
    ...
    for iter_i in range(iter_num):
        ...
        for i, (train_l_image, train_l_label) in enumerate(train_l_loader):
            ...
            prcl_loss = criterion['prcl_loss'](rep_all, uncer_all, label_all, mask_all, prob_all, prototype_mu, prototype_sigma, epoch, (i+len(train_l_loader)*iter_i))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--generalized_num', type=int, default=4)
    ...
    parser.add_argument('--generalized_radius', type=float, default=1.0)
